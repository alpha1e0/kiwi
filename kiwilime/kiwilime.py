...
'''
Kiwilime, a sublime plugin for finding security bugs.
--------------------------------------------------------------------------------
...
BTLIME_SCOPE = 'kiwilime.info'
BTLIME_SETTING_FILE = 'kiwilime.sublime-settings'
BTLIME_SYNTAX_FILE = "Packages/kiwilime/kiwilime.sublime-syntax"
...
class current(object):
    ...
    def cache_dir(cls, view):
        ...
        cachedir = settings.get("cache_directory_name", ".kiwilime-cache")
...
def is_kiwilime_info(view):
    '''
    判断是否为scope=kiwilime.info
    '''
    return view.match_selector(current.point(view), "kiwilime.info")
...
def get_file_location(view, point):
    '''
    Get the file location from the kiwilime info entry.
        For example:
            @/aaa/bbb/xxx.py
...
def get_info_entry(view, point):
    '''
    Get the kiwilime info entry from the kiwilime info entry.
    @returns:
...
def bt_analyze(view, btcmd, projdir, cachedir):
    ...
    result_file = os.path.join(cachedir, projname+".kiwi")
...
def simple_analyze(view, projdir, cachedir):
    show_error("Can not find code search tool 'kiwi',"
        " and simple mode is not support now")
...
class RunBugtrackCommand(sublime_plugin.TextCommand):
    def run(self, edit, **args):
        ...
        settings = sublime.load_settings(BTLIME_SETTING_FILE)
        btcmd = settings.get("kiwi_command", None)
